...
def display_latest_entries(context, first = 0, last = 5):
    ...
    Post:   A list of less important live entries has been rendered starting at the ith entry and ending at the jth entry, with the entries
            ordered descending by the publish date
    """
    list_of_entries = Entry.live_entries.exclude(Q(is_important = True) & Q(important_until__gte = datetime.now())).all()[first:last]
    return {'list_of_entries' : list_of_entries, 'is_first_batch' : first == 0 }
